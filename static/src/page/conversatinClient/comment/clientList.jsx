...
function ClientList(props) {
  ...
  const onItemClick = (item, index) => {
    console.log(item, index, activeIndex, '-------执行------')
    // 重复点击无效
  ...
  const itemRener = (item, index) => {
    ...
    if (props.type == 2 || props.type == 4) {
      return (
        <div
          className={`flex-box flex-column borderb-1 padl24 padr24 padt8 padb8 ${activeClassName}`}
          onClick={() => {
        ...
        >
          <div className="flex-box  flex-between">
            <div className="flex-box middle">
              ...
              <span className="text-title f14 text-bold"> {item.toUserName}</span>
          ...
          <div className="flex-box flex-between padt8 text-sub1">
            ...
            <span>{moment(item.lastMessageTime).format('YYYY-MM-DD HH:mm:ss')}</span>
          </div>
        </div>
      )
    }
        ...
        >
          <div className="flex-box  flex-between">
            <div className="flex-box middle">
              ...
              <span className="text-title f12 "> {item.toName}</span>
            </div>
            {item.outType != 1 && (
              <div
                style={{ background: 'rgba(255, 108, 121, 0.15)', height: '18px', lineHeight: '18px' }}
  ...
  return (
    <div className="flex flex-column">
      <div className="flex middle-a pad16">
        <Input
      ...
      <div className="full over-y padb20 flex1" style={{ height: '666px' }} id="scrollableDiv">
        {dataList.length == 0 ? (
