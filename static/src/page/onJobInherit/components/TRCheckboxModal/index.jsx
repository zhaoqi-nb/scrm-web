...
class ModalComponent extends React.Component {
  ...
  _onChangeAll = (e) => {
    ...
    const checkedNodes = checkAll ? [...this.leafNodes] : [];
  ...
  _onCheckTree = (val) => {
    if(val.length>1 && this.ifRadio) val = [val[1]]
    const checkedNodes = this.leafNodes.filter((x) => val.includes(x.key));
    let checkedKeys = checkedNodes.map((x) => x.key);
  ...
  render() {
    ...
    return (
      <Modal
        ...
        onOk={this._onOk}
      >
        <div className="modal_body_onJob">
